...
- **Multi-provider Git sync (Extension 3.0 parity):** GitHub (incl. Enterprise), GitLab, Gitea, Forgejo, Codeberg, and Gogs. Provider capability map, `gitProvider` + `serverUrl` profile fields, three write strategies (Git Data API tree, GitLab atomic commits, Gitea Contents API).
...
- **Settings → Git:** Provider picker, optional server URL, provider-specific token hints. Branch `develop/3.0` tracks Extension 3.0 compatibility; app release version is **0.4.0**.
...
- **Interop:** [docs/BOOKMARK-FORMAT.md](docs/BOOKMARK-FORMAT.md), [docs/EXTENSION-SYNC-VERIFY.md](docs/EXTENSION-SYNC-VERIFY.md), and [ARCHITECTURE.md](ARCHITECTURE.md) updated for Extension **3.0.x** and multi-provider sync.
